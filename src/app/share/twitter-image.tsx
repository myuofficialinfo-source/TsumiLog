...
export default async function Image({ searchParams }: { searchParams: { [key: string]: string } }) {
  const catchphrase = searchParams.catchphrase || 'ゲーマー';
  const totalGames = searchParams.totalGames || '0';
  const backlogCount = searchParams.backlogCount || '0';
  const playtime = searchParams.playtime || '0';
